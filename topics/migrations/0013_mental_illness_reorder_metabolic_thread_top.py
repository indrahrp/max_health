...
<p>The brain consumes roughly 20% of total energy despite representing only 2% of body weight.
When that energy supply is disrupted — by glucose dysregulation, mitochondrial dysfunction,
neuroinflammation, or insulin resistance — brain circuits malfunction in ways we label as
psychiatric illness. Understanding why ketones reach the brain when other interventions cannot
is the key to understanding what follows.</p>

""" + BBB_SVG + """

<p>Dietary fat and stored body fat (triglycerides) cannot directly fuel neurons — the brain has
minimal capacity to oxidise fatty acids. Instead, the liver performs β-oxidation: it cleaves
fatty acids into acetyl-CoA and packages the surplus into ketone bodies — primarily
beta-hydroxybutyrate (BHB) and acetoacetate. These small, water-soluble molecules cross the
blood-brain barrier freely via MCT1 transporters, entering neurons where they are converted
back into acetyl-CoA and fed into the mitochondrial energy cycle.</p>

<p>The result is a more efficient fuel — ketones yield approximately 25% more ATP per unit of
oxygen than glucose, while simultaneously raising GABA (the brain's primary calming
...
with a direct, efficient fuel source. Marcus also eliminated ultra-processed food, added omega-3s,
and walked 30 minutes each morning.</p>
...
can occasionally be activating in some bipolar patients).</p>
...
psychiatrist started a second-generation antipsychotic. Several months into her recovery, her
care team introduced a ketogenic dietary protocol as an adjunct — with careful attention to
the fact that some antipsychotic medications can impede ketosis, requiring close prescriber
collaboration.</p>
...
delay seizures by 200–600% in animal models.</p>
...
<div style="background:#0d1827;border-left:3px solid #d97706;padding:0.85em 1.2em;border-radius:0 8px 8px 0;margin:1em 0 0;">
  <p style="margin:0 0 0.55em;font-size:0.93em;line-height:1.6;"><strong style="color:#fcd34d;">Beatriz, 7</strong> — Absence epilepsy causing dozens of episodes daily, poorly controlled on two medications. A classical ketogenic diet (4:1 fat-to-protein-carbohydrate ratio) was initiated under a specialist neurological dietitian. Seizure-free within six weeks; medications gradually tapered. Her teachers describe a complete transformation in classroom attention and engagement.</p>
...
meaningful OCD symptom reduction with ketogenic dietary therapy. The proposed mechanism involves
...
def reorder_metabolic_thread_to_top(apps, schema_editor):
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('topics', '0012_mental_illness_expanded_cases_and_bbb_illustration'),
    ]

    operations = [
        migrations.RunPython(reorder_metabolic_thread_to_top, reverse_reorder),
    ]
